...
const GH_CACHE_PREFIX = "ty-gh-repos";
const GH_CACHE_TTL_MS = 1000 * 60 * 10; // 10 minutes

type RepoCachePayload = { timestamp: number; items: Repo[] };

function readRepoCache(user: string): Repo[] | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = sessionStorage.getItem(`${GH_CACHE_PREFIX}:${user}`);
    if (!raw) return null;
    const payload = JSON.parse(raw) as RepoCachePayload;
    if (!payload?.timestamp || !Array.isArray(payload.items)) return null;
    if (Date.now() - payload.timestamp > GH_CACHE_TTL_MS) {
      sessionStorage.removeItem(`${GH_CACHE_PREFIX}:${user}`);
      return null;
    }
...
function writeRepoCache(user: string, items: Repo[]) {
  if (typeof window === "undefined") return;
  try {
    const payload: RepoCachePayload = { timestamp: Date.now(), items };
    sessionStorage.setItem(`${GH_CACHE_PREFIX}:${user}`, JSON.stringify(payload));
  } catch {
...
export const Projects: React.FC<{ config?: Config }> = ({ config }) => {
  ...
  useEffect(() => {
    ...
    const cacheKey = gh.user.toLowerCase();
    const cachedRepos = readRepoCache(cacheKey);
    if (cachedRepos) {

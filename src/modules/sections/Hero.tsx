...
export const Hero: React.FC<{ data: SiteData }> = ({ data }) => {
  ...
  useEffect(() => {
    ...
    const onWindowChange = () => updateBounds();
  ...
  const processPointer = () => {
    ...
    const xRaw = point.x - bounds.left;
    const yRaw = point.y - bounds.top;
    const x = Math.min(Math.max(xRaw, 0), width);
    const y = Math.min(Math.max(yRaw, 0), height);

    mx.set(((x / width) * 2 - 1) * 50);
    my.set(((y / height) * 2 - 1) * 50);
